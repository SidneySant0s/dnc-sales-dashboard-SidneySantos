...
import { CardComponent, CustomTable, FormComponent,Header, StyledH2, StyledP, StyledSpan, StyledButton } from "@/components"
...
//HOOKS
  ...
  const { deleteData: leadsDeleteData, loading: leadsDeleteLoading } = useDelete()
  ...
  const handleDelete = async (id:number) => {
      ...
      try {
        await leadsDeleteData(`leads/delete/${id}`)
  ...
  useEffect(() =>{
    if(createLeadsData){
      ...
      setCreateMessage({
        msg: 'Não foi possivél realizar a operação. Entre em contato com nosso suporte.',
        type: 'error'
      })
      getLeads()
      clearMessage()
